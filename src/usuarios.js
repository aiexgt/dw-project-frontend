...
const mostrar = (token, word) => {
  if (word != "" && word != null && word != undefined) {
    word = "http://34.125.111.145:80/usuario?word=" + word;
  } else {
    word = "http://34.125.111.145:80/usuario";
  }
...
const add = () => {
  ...
  if (
    ...
    fetch("http://34.125.111.145:80/usuario", requestOptions)
      .then((response) => response.text())
      .then((result) => {
        const info = JSON.parse(result);
        if (result.error) {
...
const selectRol = (token) => {
  ...
  fetch("http://34.125.111.145:80/usuario/rol", requestOptions)
    .then((response) => response.text())
    .then((result) => {
      const info = JSON.parse(result);
...
const eliminar = (id) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      fetch("http://34.125.111.145:80/usuario/" + id, requestOptions)
        .then((response) => response.text())
        .then((result) => {
          if (result != "") {
            Swal.fire({
...
const editarUsuario = () => {
  ...
  if (
    ...
    fetch("http://34.125.111.145:80/usuario/"+usuarioId, requestOptions)
      .then((response) => response.text())
      .then((result) => {
        const info = JSON.parse(result);
        if (result.error) {
