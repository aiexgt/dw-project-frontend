...
const mostrar = (token, word) => {
  if (word != "" && word != null && word != undefined) {
    word = "http://34.125.111.145:80/articulo?word=" + word;
  } else {
    word = "http://34.125.111.145:80/articulo";
  }
...
const add = () => {
  ...
  if (
    ...
    fetch("http://34.125.111.145:80/articulo", requestOptions)
      .then((response) => response.text())
      .then((result) => {
        const info = JSON.parse(result);
        if (result.error) {
...
const eliminar = (id) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      fetch("http://34.125.111.145:80/articulo/" + id, requestOptions)
        .then((response) => response.text())
        .then((result) => {
          if (result != "") {
            Swal.fire({
...
const editarUsuario = () => {
  ...
  if (
    ...
    fetch("http://34.125.111.145:80/articulo/"+articuloId, requestOptions)
      .then((response) => response.text())
      .then((result) => {
        const info = JSON.parse(result);
        if (result.error) {
